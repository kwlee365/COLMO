#!/usr/bin/env python3
"""
XsensToCOLMO Adapter
Converts Xsens MVN real-time streaming data to COLMO-compatible format for humanoid robot retargeting.
"""
...
try:
    ...
    # Optional: only needed for live Xsens MVN streaming. Kept soft so the module (and
    # scripts/xsens_live_streaming.py --help) imports without the UDP parser installed.
    XsensWrapper = None


class XsensToCOLMO:
    """
    Adapter class to convert Xsens MVN streaming data to COLMO human_frame format.
    ...
    # Mapping from Xsens link names to COLMO expected body names
    # Based on actual Xsens MVN output (lowercase with underscores)
    XSENS_TO_COLMO_MAPPING = {
        # Core body (REQUIRED)
        'pelvis': 'Pelvis',
        'l5': 'Spine',
        'l3': 'Spine1',
    ...
    def __init__(self, port: int = 8001, verbose: bool = True):
        """
        Initialize the Xsens to COLMO adapter.
        
        Args:
            port: UDP port for Xsens MVN streaming (default: 8001)
            verbose: Print debug information
        """
        if XsensWrapper is None:
            raise ImportError(
                "xsens_mvn_robot is required for live Xsens MVN streaming. See the "
                "'Install the Xsens MVN UDP Data Parser' section of README.md."
            )
    ...
    def _build_available_mappings(self):
        ...
        for xsens_name, colmo_name in self.XSENS_TO_COLMO_MAPPING.items():
            if xsens_name in self.xsens_link_names:
                self.available_mappings[colmo_name] = xsens_name
        
        if self.verbose:
            print(f"\n✅ Found {len(self.available_mappings)} mapped bodies:")
            for colmo_name, xsens_name in self.available_mappings.items():
                is_required = colmo_name in self.REQUIRED_BODIES
                marker = "⭐" if is_required else "  "
                print(f"  {marker} {colmo_name:20s} <- {xsens_name}")
    ...
    def get_human_frame(self) -> Optional[Dict[str, Tuple[np.ndarray, np.ndarray]]]:
        """
        Get current frame data in COLMO format.
        ...
        for colmo_name, xsens_name in self.available_mappings.items():
            try:
                ...
                # Add to human frame
                human_frame[colmo_name] = (pos, ori_scalar_first)
                
            except Exception as e:
                if self.verbose:
                    print(f"⚠️  Warning: Could not get data for {colmo_name} ({xsens_name}): {e}")
                continue
...
def test_xsens_to_colmo(port: int = 8001, duration: float = 5.0):
    """
    Test function to verify XsensToCOLMO adapter.
    ...
    print("="*70)
    print("🧪 Testing XsensToCOLMO Adapter")
    print("="*70)
    
    # Initialize adapter
    adapter = XsensToCOLMO(port=port, verbose=True)
...
if __name__ == '__main__':
    ...
    # Run test
    test_xsens_to_colmo(port, duration)
