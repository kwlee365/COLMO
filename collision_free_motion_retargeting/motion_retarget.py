...
class CollisionFreeMotionRetargeting:
    ...
    def __init__(
        ...
        ik_config: str = None,           # path override; None -> IK_CONFIG_DICT lookup
    ) -> None:
        ...
        # Load the IK config
        # An explicit path overrides the (src_human, tgt_robot) registry entry. This is what
        # key-body ablations use: the variant JSON lives outside IK_CONFIG_DICT, and adding a
        # pseudo-robot key instead would break the ASSET_ROOT/<tgt_robot>/collision_cfg.yaml
        # lookup below (there is no assets/<variant>/ directory).
        ik_config_path = ik_config if ik_config is not None \
            else IK_CONFIG_DICT[src_human][tgt_robot]
        with open(ik_config_path, encoding="utf-8") as f:
            ik_config = json.load(f)
        self.ik_config_path = str(ik_config_path)
        if verbose:
